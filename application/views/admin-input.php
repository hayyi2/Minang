		...
		<div class="row">
			...
			<div class="col-sm-6">
				...
				<?php endif ?>
				<h4>
					<a href="<?php url('admin') ?>" class="text-secondary"><i class="fa fa-fw fa-arrow-left"></i></a>
					<?php echo (($mode == 'add') ? 'Input' : 'Edit'); ?> Admin
				</h4>
				<hr>
				<form method="post" action="<?php url(($mode == 'add') ? 'admin/input' : 'admin/edit/'.$data->user_id) ?>">
					...
					<div class="form-group row">
						...
						<div class="col-sm-8">
							<button type="submit" class="btn btn-primary"><?php echo (($mode == 'add') ? 'Add New' : 'Edit'); ?> Admin</button>
							<?php if ($mode == 'edit'): ?>
								<a href="<?php url('admin/input') ?>" class="btn btn-secondary">Add New Admin</a>
							<?php endif ?>
						</div>
					</div>
				</form>
